...
Chapter 7 points: 2 + 4 + 4 = 10 points total. 

#\textcolor{cyan}{Easy (2 points total)}

##\textcolor{cyan}{7E1} 

a) There's an interaction between yeast and sugar. 
b) A third variable would be choice of degree. Some education doesn't lead to higher income. 
c) Gasoline makes a car go... conditional on the car being turned on. 

##\textcolor{cyan}{7E2}

Only the first one is an interaction since it's the only one that can be reworded using "conditional". 

##\textcolor{cyan}{7E3}

a) $$\mu_i = \alpha + \beta_{H}\text{heat} + \beta_D \text{dry} + \beta_{HD} \text{heat} \cdot \text{dry} $$
b) $$\mu_i = \alpha +\beta_c \text{cylinder} + \beta_f \text{fuel injector} $$
c) $$\mu_i = \alpha  + \beta_P \text{parents} + \beta_F \text{friends} $$
d) $$\mu_i = \alpha + \beta_S \text{social} + \beta_A \text{appendages} $$ 

#\textcolor{cyan}{Medium (4 points total)}

##\textcolor{cyan}{7M1 (2 points)}

This is because there is a three-way interaction. This means that in order for tulips to bloom, temperature, water and shade must all interact, and hot temperatures do not support tulips. 

##\textcolor{cyan}{7M2 (2 points)}

Just multiply by an indicator variable. It it's hot out, let's say that $H = 0$. Then the regression equation would be $\mu = LH$, where $L$ is the linear equation that we already established in the chapter. 
